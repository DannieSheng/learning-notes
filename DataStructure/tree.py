...
class Solution(object):
    ...
    def levelOrder(self, root: TreeNode) -> List[List[int]]:  # output layer by layer [[3]，[9，20]，[15，7]]
       if not root: return []
       res, queue = [], [root]
       while queue:
           arr, size = [], len(queue)
    ...
    def levelOrderSimplified(self, root: TreeNode) -> List[List[int]]:  # simplified output: [3, 9, 20, 15, 7]
        res, queue = [], [root]
        while queue:
            cur = queue.pop(0)
            res.append(cur.val)
